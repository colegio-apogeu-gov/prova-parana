...
export const enrichWithComponentesHabilidades = async (data: any[]) => {
  if (!data || data.length === 0) return data;

  const combos = new Set<string>();
  data.forEach((row) => {
    if (row.componente && row.ano_escolar && row.habilidade_id) {
      combos.add(`${row.componente}||${row.ano_escolar}||${row.habilidade_id}`);
    }
  });
  if (combos.size === 0) return data;
...
export const uploadProvaData = async (data: any[]) => {
  const enriched = await enrichWithComponentesHabilidades(data);
...
// Retorna lista distinta de ano_prova encontrados na tabela prova_resultados,
// opcionalmente filtrando pela unidade.
export const getAnosProva = async (unidade?: string): Promise<string[]> => {
  try {
