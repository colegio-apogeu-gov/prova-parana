...
const getColorByValue = (v: number) => {
  if (v >= 70) return '#10B981'; // verde
  if (v >= 30) return '#F59E0B'; // amarelo
...
const ProficiencyGauge: React.FC<ProficiencyGaugeProps> = ({
  ...
  const clamp = (x: number) => (Number.isFinite(x) ? Math.min(Math.max(x, 0), 100) : 0);
  const v = clamp(value);

  const color = getColorByValue(v);
  ...
  const centerY = resolvedHeight - strokeWidth / 2;
  ...
  const redPath = arc(180, 130);
  const yellowPath = arc(130, 50);
  const greenPath = arc(50, 0);
