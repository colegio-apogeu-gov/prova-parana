...
import { PresetEdicoes } from '../../lib/enemComparacao';

interface Props {
  base: string;
  comparada: string;
  opcoesEdicao: string[];        // edições oferecidas nos selects base/comparada
  chartEdicoes: string[];        // todas as edições (chips do histórico)
...
// Filtro base × comparada + chips de edições (com atalhos Todas / Últimas 3 / Só
// o par). Análogo ao do IDEB, no tema emerald do ENEM.
const EnemComparacaoControls: React.FC<Props> = ({
      ...
      <div className="flex items-end gap-2">
        <div>
          ...
          <select value={base} onChange={(e) => onBase(e.target.value)}
            className="px-2 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500">
      ...
      <div className="flex items-center gap-1 shrink-0 self-start xl:self-center text-xs">
        {([
          ...
          <button key={o.k} onClick={() => onPreset(o.k)}
            className="px-2.5 py-1 rounded-lg text-gray-500 hover:text-emerald-700 hover:bg-emerald-50 font-medium transition-colors">
            {o.label}
          </button>
        ))}
      </div>
...
export default EnemComparacaoControls;
